...
self.addEventListener('fetch',function(e){
  ...
  if(req.mode==='navigate'||(req.headers.get('accept')||'').indexOf('text/html')>-1){
    e.respondWith(fetch(req).then(function(res){var cl=res.clone();caches.open(C).then(function(c){c.put('./index.html',cl)});return res}).catch(function(){return caches.match('./index.html')}));
    return;
  }
  e.respondWith(caches.match(req,{ignoreSearch:true}).then(function(r){return r||fetch(req).then(function(res){var cl=res.clone();caches.open(C).then(function(c){c.put(req,cl)});return res})}));
});
